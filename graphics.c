...
/* Exported entries */
void putImg(char picture_path[],int paint_x,int paint_y,int PixelWidth,int PixelHeight){
	HINSTANCE hInstanc=GetModuleHandle(NULL);
	//1. 加载图片
	HANDLE PhotoHdc=LoadImage(hInstanc,(LPCWSTR)picture_path,IMAGE_BITMAP,0,0,LR_LOADFROMFILE);
	//2. 创建兼容DC
	HDC bufhdc=CreateCompatibleDC(NULL);//与指定设备兼容的DC
	//3. 把图片选择到兼容DC
	SelectObject(bufhdc,PhotoHdc); //将对象选择到按指定的设备上下文中
	//4. 从兼容DC绘制到HDC
	BitBlt(osdc,paint_x,paint_y,1e9,1e9,bufhdc,0,0,SRCCOPY);
	InvalidateRect(graphicsWindow,NULL,TRUE); 
	DeleteDC(bufhdc);
	DeleteObject(PhotoHdc);
	DeleteDC(hInstanc);
}
void MyQuit(){
                 ...
                 double red, double green, double blue)
{
    ...
    FreeBlock(colorTable[cindex].name);
